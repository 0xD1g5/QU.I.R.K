...
ERROR_REGISTRY: dict[str, ErrorEntry] = {
    ...
    "CONFIG-002": ErrorEntry(
        code="CONFIG-002",
        cause="A connectors.broker_targets entry could not be parsed as host or host:port: the "
        "host:port string is unparseable (e.g. empty host, ambiguous unbracketed IPv6 with "
        "multiple colons), or the port half is not a valid TCP port (not a number, a "
        "non-integral number, or outside the range 1-65535).",
        fix="Use a bare hostname/IP (e.g. \"kafka.internal\"), \"host:port\" (e.g. "
