...
HARDWARE_MATRIX = {
    ...
    "entries": [
        ...
        },
        {
            ...
            "pqc_status": "unsupported",
            "eol_date": None,
            # NOT re-verified on 2026-09-13 \u2014 deliberately left at the prior date. See notes.
            "last_verified": "2026-06-13",
            ...
            "notes": "IPMI 2.0 session security (RMCP+/RAKP) is widely understood to draw its authentication, integrity and confidentiality algorithms from a CLOSED, ENUMERATED cipher-suite table fixed in the specification, with no algorithm-negotiation extension point, and the specification is terminal at v2.0 rev 1.1 \u2014 so no PQC algorithm can be added without a successor spec. NOT REMEDIABLE IN PLACE: migrate to Redfish (HTTPS/TLS, which inherits PQC from the TLS stack rather than defining its own) or disable IPMI-over-LAN. PENDING VERIFICATION: the closed-cipher-suite-table claim above has NOT been read against the specification and this entry's last_verified is deliberately NOT bumped until it has been; the recorded source_url is also still the known-bad Intel page, which returns HTTP 200 serving a generic Server Products selector on the wrong subject. pqc_status was changed from VENDOR-SILENT to unsupported on 2026-09-13 WITHOUT source verification, deliberately: VENDOR-SILENT plus high confidence resolves to Tier 3 in hardware_tier.assign_tier \u2014 the same remediation tier as a fully PQC-capable device \u2014 so a banner reading 'IPMI 2.0' scored LOWER urgency than a bare 'IPMI' banner. Moving to unsupported (Tier 1) is strictly more conservative and fails safe; it is not an attestation. The prior notes also asserted that individual BMC vendors have made no public PQC statements \u2014 a negative-existence claim across an open set of vendors that no single document can ever support \u2014 and that clause has been removed rather than re-sourced. NOTE this row is keyed on a PROTOCOL, not a vendor: it is the only such entry of the eight, and Intel is the specification author rather than the subject.",
        },
        {
            "vendor": "Thales",
