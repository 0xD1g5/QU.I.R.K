...
def _ensure_hardware_drift_events_table(engine) -> None:
    """Phase 155 HWLC-04: create hardware_drift_events table if absent (idempotent).

    HardwareDriftEvent is registered on Base.metadata via import of quirk.models.
    Uses Base.metadata.create_all with checkfirst=True — same pattern as
    _ensure_merge_runs_table. New table only — not new columns.
    """
    Base.metadata.create_all(engine, checkfirst=True)


def _ensure_vendor_pqc_trend_events_table(engine) -> None:
    """Phase 160 HWLC-17: create vendor_pqc_trend_events table if absent
    (idempotent).

    VendorPqcTrendEvent is registered on Base.metadata via import of
    quirk.models. Uses Base.metadata.create_all with checkfirst=True — same
    pattern as _ensure_hardware_drift_events_table. New table only — NOT new
...
def _ensure_scan_phase_records_table(engine) -> None:
    """Phase 192 OBS-01: create scan_phase_records table if absent (idempotent).

    ScanPhaseRecord is registered on Base.metadata via import of
    quirk.models. Uses Base.metadata.create_all with checkfirst=True — same
    pattern as _ensure_remediation_closure_events_table. New table only —
    NOT new columns, so _ensure_columns() / _ADDITIVE_MIGRATIONS
    deliberately do not apply here.
    """
    Base.metadata.create_all(engine, checkfirst=True)
...
def init_db(db_path: str) -> Engine:
    ...
    _ensure_scan_phase_records_table(engine)  # Phase 192 — OBS-01
