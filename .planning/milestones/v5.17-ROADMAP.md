...
- [x] 176-07 (addendum, no PLAN.md — added by explicit user direction at 176-06's checkpoint,
  ...
  that session, so the tally stood at 9/13 PASS, 2 BACKLOG, 2 GAP. Superseded by 176-08; see
  `176-07-SUMMARY.md`.
- [x] 176-08 (addendum, no PLAN.md — GAP-closure re-run, 2026-08-31) — Corrected the `ssh-audit`
  invocation and re-ran `UAT-5-11`/`UAT-6-08` with the lab up, closing both LABRUN-01 GAPs.
  **Final tally: 10 PASS / 3 FAIL / 0 GAP** of 13 cases. Surfaced and fixed TRIAGE-176-03
  (`quirk/scanner/ssh_scanner.py` passed two positionals to a tool taking one `host:port`, so
  every SSH scan silently degraded to a banner grab with `ssh_audit_json` NULL) with an
  argv-asserting regression test; TRIAGE-176-01/02 backlogged. Commit `2dedf0dc`; see
  `176-08-SUMMARY.md`.
