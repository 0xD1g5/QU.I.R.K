# Requirements Archive: v4.5 Reliability & Gap Closure
...
- [x] **GAP-03**: Phase 36 `wave_0_complete: false` is flipped to `true` in `36-VALIDATION.md` and the matrix shows `nyquist_compliant: true, wave_0_complete: true` (closes DEF-v4.4-01).
- [x] **GAP-04**: User opens the dashboard and sees a "Data at Rest" tab alongside the existing tabs, listing DB / object storage / K8s secrets / Vault findings with the existing v4.3 data shape (closes DASH-05 deferred from Phase 27).
...
- [x] **CBOM-03**: Golden snapshot drift between v4.4 and v4.5 is reviewed; any change is intentional, documented, and accompanied by a snapshot update commit with rationale.
...
| GAP-03 | Phase 38 | Complete |
| GAP-04 | Phase 39 | Complete |
