...
| BACK-91 | Chaos lab — macOS host-mount compat (3 services restart/crash on macOS Docker Desktop) | P2 — chaos lab macOS portability | Discovered 2026-05-15 during Phase 999.83 Plan 05 global UAT (`./lab.sh down && ./lab.sh all`, 60s settle). With all 4 BACK-90 bugs fixed, 3 pre-existing services still fail on macOS hosts only — all rooted in the same problem class: Docker Desktop on macOS exposes bind-mounted host files with different uid/gid + permission semantics than Linux Docker, and these services rely on chowning or writing to those bind-mounts. **(1) DEF-999.83-A — `ldaps` profile** — `chaoslab-ldaps-1` (osixia/openldap:1.5.0) enters restart loop with `chown: ... Read-only file system` on `/container/service/slapd/assets/certs/...`. The image startup script tries to chown bind-mounted cert files; macOS Docker Desktop exposes them read-only. **Fix options:** (a) switch base image to one without the chown step (e.g. `bitnami/openldap`); (b) copy certs into a named volume in an init container before slapd starts; (c) pass `KEEP_EXISTING_CONFIG=true` env if image supports skipping the chown. **(2) DEF-999.83-B — `broker` profile** — `chaoslab-rabbitmq-broker-1` exits with `Error when reading /var/lib/rabbitmq/.erlang.cookie: eacces` then kernel-pid crash. RabbitMQ requires the erlang cookie file to be 0400 root:root, but macOS Docker Desktop bind-mount uid/gid mapping breaks this. **Fix options:** (a) set `RABBITMQ_ERLANG_COOKIE` env so RabbitMQ never reads the file; (b) use a named volume instead of bind-mount for `/var/lib/rabbitmq`; (c) move cookie creation into a Dockerfile RUN so the file is baked into the image with correct perms. **(3) DEF-999.83-C — `source` profile (gitea-seed idempotency)** — `chaoslab-gitea-seed-1` exits with code 22 on every re-run after the first because `curl -sf POST /api/v1/user/repos` returns 409 (repo exists) and `set -e` kills the script. Not strictly a macOS-host issue but surfaced in the same UAT — symptom only visible when the gitea named volume persists across `lab.sh down && lab.sh all` cycles. **Fix:** wrap each `POST /api/v1/user/repos` and `put_file` curl call in `|| true` or check for repo/file existence first via `GET /api/v1/repos/{user}/{repo}` (and per-path GET) before POSTing. Verified the seed effectively succeeded — repos and files DO exist after first run; the script just exits noisily on re-run. **Verification (after all 3 fixed):** `./lab.sh down && ./lab.sh all && sleep 60 && docker compose -p chaoslab ps` shows zero `Restarting`, zero `Exited (1)`, zero `Exited (22)` on macOS Docker Desktop. ldaps stays `Up`; rabbitmq-broker stays `Up (healthy)`; gitea-seed `Exited (0)` on both first AND subsequent runs without wiping the gitea_data volume. **Out of scope:** generic "audit all bind-mounts in docker-compose.yml for macOS compat" — focus on these 3 specific services with documented failure modes. **CLAUDE.md chaos-lab rule:** if image pins change (ldaps base image), update lab.sh / README.md / expected_results_v4.md together. **Source:** `.planning/phases/999.83-chaos-lab-service-config-drift/deferred-items.md`. Phase dir: `.planning/phases/999.84-chaos-lab-macos-mount-compat/` |
| BACK-90 | Chaos lab — fix 4 service config drift bugs unmasked by `lab.sh all` | P2 — chaos lab reliability | Discovered 2026-05-15 after BACK-89 work made `lab.sh all` reach completion on macOS for the first time. With kerberos skip + vault healthcheck fixed, 59 of 64 containers come up; 5 services still fail with pre-existing config drift. **(1) gitea (profile: source)** — `gitea/gitea:1.21` refuses to run as root since 1.21.x. Compose's custom `command: bash -c "gitea web && gitea admin user create..."` bypasses the image entrypoint's user-drop logic. Fix: remove the bash wrapper, let the entrypoint run gitea as the `git` user, and move admin user creation into `source/seed.sh` (it already waits for gitea healthcheck; just `curl` the admin-create API or run `docker exec gitea ...`). Log evidence: `[F] Gitea is not supposed to be run as root`. **(2) minio-seed (profile: storage-s3)** — fails with `Unable to enable auto encryption. Server side encryption specified but KMS is not configured.` The seed script issues `mc admin config set local kms ...` or `mc encrypt set sse-s3 ...` without first configuring minio with `MINIO_KMS_SECRET_KEY` env. Fix: either set the KMS env vars on the `minio` service (consulting-grade: use `MINIO_KMS_SECRET_KEY=lab-key:<base64-32-bytes>`) or strip the auto-encryption step from `storage/seed.sh` if it's not needed for current findings. **(3) vault-seed (profile: storage, deprecated)** — fails on `PUT /v1/transit/keys/rsa-1024 → 400 unknown key type rsa-1024`. Vault's transit engine doesn't accept `rsa-1024` (valid: `rsa-2048/3072/4096`, `aes256-gcm96`, `ecdsa-p256` etc.). The seed script in `vault/seed.sh` likely tries to create a weak-key target for QUIRK's vault scanner to flag. Real fix: use `rsa-2048` (which still triggers QRAMM weak-RSA finding for sub-3072 keys per the model) OR if rsa-1024 is needed specifically, generate the key externally and import via `transit/keys/<name>/import` with a raw 1024-bit RSA key. **(4) mysql-ssl-off (profile: database)** — fails with `unknown option '--skip-ssl'`. The compose command uses `--skip-ssl` which was removed in MySQL 8.4 (`mysql:8.4.9` per logs). Fix: pin to `mysql:8.0` (last version supporting `--skip-ssl`) OR migrate to MySQL 8.4's replacement: `--require-secure-transport=OFF` + remove `--skip-ssl`. Recommend pinning to 8.0 since the lab's purpose is to expose insecure-by-default behavior — 8.4 changing the syntax is a moving target. **Verification:** after fix, `./lab.sh down && ./lab.sh all` produces 64 running containers with all seeds Exited(0). Expected findings in `expected_results_v4.md` for profiles `source`, `storage-s3`, `storage`, `database` should match the same scanner output as before. **Note:** the deprecated `storage` profile (per README.md row 33) may be removed entirely instead of fixing vault-seed — call this out during planning. Phase dir: `.planning/phases/999.83-chaos-lab-service-config-drift/` (promoted to active 2026-05-15) |
| BACK-89 | Chaos lab — remap kerberos profile host port off 88 + make scanner port configurable | P2 — chaos lab macOS portability | Discovered during v4.8 hardening (2026-05-15) when `./lab.sh all` failed with `bind: address already in use` on port 88. **Root cause:** macOS ships a built-in Heimdal KDC bound to `*:88` that cannot be unbound without disabling system Kerberos. The `samba-dc` service in `quantum-chaos-enterprise-lab/docker-compose.yml:851` publishes `88:88` directly (privileged port, not remapped like every other lab service which uses high 2xxxx ports). **Workaround in place (2026-05-15):** `lab.sh all` now skips the `kerberos` profile on Darwin unless `LAB_INCLUDE_KERBEROS=1` is set. Linux hosts are unaffected. This is a portability bug Phase 40 RESEARCH explicitly deferred (`40-RESEARCH.md:312`: "Whether the `kerberos` profile's privileged-port binding (88, 389) is OK as-is or should be remapped... out-of-scope per 'no compose changes'"). **Full fix scope:** (1) compose remap `88:88` → `8088:88` and `389:389` → `3389:389` for symmetry; (2) make `quirk/scanner/kerberos_scanner.py` accept a non-default port — currently hardcodes `88` at lines 160, 290, 318, 344 (`sock.sendto(message, (host, 88))` and three `Endpoint(..., port=88, ...)` records); accept `host:port` syntax in `--targets` and override; (3) update `config_template.yaml:64` comment about kdc port; (4) update `expected_results_v4.md:325, 333, 337, 338, 343` (port table + scan example); (5) update `README.md:38` (port column); (6) remove the Darwin-skip from `lab.sh`; (7) refresh `docs/chaos-lab.md` port reference; (8) UAT pass: `./lab.sh all` on macOS starts kerberos with no port conflict and `quirk scan --targets localhost:8088` produces the same Kerberos etype findings (rc4-hmac HIGH, aes128 HIGH) as the previous port-88 oracle. **Decision required during planning:** whether to keep the real-world Kerberos-on-88 assumption in the scanner default and just allow override, or change the default to 8088 (which would break out-of-the-box scans of real KDCs). Recommend keeping default 88 — chaos lab is the exception, not the rule. Phase dir: `.planning/backlog/999.82-lab-kerberos-port-remap-macos/` |
