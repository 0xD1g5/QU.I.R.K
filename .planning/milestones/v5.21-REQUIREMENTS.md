# ARCHIVED — Milestone v5.21 Requirements (closed 2026-09-10)

> Archived at milestone close. All 13 v1 requirements satisfied (audit 13/13, tech_debt accepted).
> Outcomes: 12 validated as specified; PARITY-04 adjusted in-flight (SSH port list dropped per
> Phase 194 D-18/999.106 — no CLI counterpart). Tier B of the Exposure Map (reachability
> rendering) deferred by MAP-01 spike DECISION: DEFERRED → backlog 999.107; MAP-01 itself is
> satisfied (the requirement was the gate decision, not Tier B delivery).
