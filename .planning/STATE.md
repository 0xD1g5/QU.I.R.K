...
|----------|------|----------|--------|
| todo | gsd-phase-complete-premature-completion | high | open — phase.complete/milestone.complete remain UNSAFE on this machine |
| todo | gsd-state-planned-phase-misleading-empty-updated | medium | open |
| todo | gsd-state-bold-field-search-unscoped-latent | medium | open — dormant, deliberately unfixed with stated reason |
| quick_task | 260611-g0b-merge-healthcare-vertical-branch-into-ma | — | known false positive (complete; audit-open misreports since v5.10). Do not re-investigate. |

Non-blocking tech debt recorded in `.planning/milestones/v5.22-MILESTONE-AUDIT.md`: combined
connectors+advanced overlay CI regression test; 198-review IN-01/IN-02 accepted-advisory;
chaos-lab Docker network-race environmental class. Unpushed `main` commits batch-push pending
operator go-ahead at this close.
...
**Current focus:** v5.23 Phase 199 (Wave A Correctness Drain) COMPLETE 2026-09-11 — verification `passed` 8/8; TRIAGE-10/TRIAGE-11 both `[x]` (fractional scores round-trip via Optional[float] widening across merge/trends/scan transports + SubscoreSlot null-honest gauges; combined connectors+advanced overlay regression test green); full suite 4845 passed / 0 failed; code review resolved (4 warnings + 5 infos fixed, commits cd777242/a8181d5b/8867bd91/e8918400); UAT Series 199 added (1 honest GAP: UAT-199-05 em-dash browser rendering); backlog-reconciliation gate re-greened after v5.22-archive drift (c2d4ee81). Percent is phase-based (1/4) this milestone. Phase 200 (Report Branding & Templates) COMPLETE 2026-09-11 — verification `human_needed`→operator-approved (5/5 must-haves; UAT-200-01/02/11 approved in a live branded-report walkthrough); RPT-01..05 all `[x]`; SSTI gate GO (13/13 payloads contained under unconditional SandboxedEnvironment); 999.105 Tier 2 NO-GO recorded in HORIZON (RPT-F-01 stays spike-gated); report profiles + `--report-profile` CLI shipped; code review resolved (3 warnings + 4 infos fixed incl. the WR-03 SSTI-marker vacuity, 2 accepted-advisory); full suite 4909/0 (empty failing SET); TRIAGE-149 order-pollution avoided by patching the PDF leg at the writer seam in the new test files. Also this session: 999.108 (main-CI-red connector-gate regressions) fixed+closed (cfdd5438), 999.109 (static release bodies) filed P2. Phase 201 (Score-Lift Roadmap Re-frame) COMPLETE 2026-09-12 — verification `passed` 5/5; LIFT-01..LIFT-05 all `[x]`; BACK-51 CLOSED by recorded decision in HORIZON. 8/8 plans. Post-execution chain all run: code review 0 critical / 2 warning / 2 info (WR-01 fixed — `build_exec_markdown` was double-rescoring every report and discarding the result, dead on both branches; WR-02 + both infos accepted-advisory), UI review 20/24 with all 4 warnings fixed (`formatScoreNumber` per UI-SPEC in both roadmap.tsx and print.tsx, `score_lift`/`projected_score` widened to required `number | null`, real `flex gap-1.5` badge row, off-grid `ml-1.5` deleted; statics rebuilt, 351 vitest tests). **One genuine in-phase regression found by distrusting a subagent's attribution:** 201-08 filed 7 full-suite failures in `tests/test_score_lift_report_surfaces.py` as "pre-existing", but `git log --diff-filter=A` showed plan 201-05 created that file in this phase against an EMPTY documented baseline — TRIAGE-149 Playwright order pollution, fixed at the writer seam per Phase 200's precedent (`d1f1351f`). Full suite 4960 passed / 1 failed; failing SET is exactly {`test_hardware_matrix_not_stale`}, a 91-day calendar-time trip, operator-deferred with a record (todo `hardware-matrix-staleness-reverify.md`) — the date was NOT bumped, since bumping without re-verifying fabricates the attestation. Also this session: CLAUDE.md §Staleness Review Cadence corrected (it said "eight" catalogs and omitted `hardware_meta.py`, the very one that tripped, plus two undocumented generator-drift gates `score-strings.json`/`severity-bands.json`; the section now leads with enumeration commands and an instruction not to trust its own list). ROADMAP progress-table rows for 199/200 were stale at `0/? | Not started` despite both being `[x]` with dates — corrected to 5/5 and 7/7 Complete in the same pass. 4 deferred items recorded for 201, incl. item 4: `compute_item_lifts` truncates via `int(delta)` (`score_lift.py:164`) so fractional lifts under-report and the new frontend fractional branch is unreachable — deliberately NOT fixed (it moves an operator-approved displayed number on four surfaces; needs its own decision + re-walkthrough). Next: Phase 202 (Finding Storyline Drawer) — last phase of v5.23, no directory yet, needs discuss → ui-phase → plan → execute. Scope: Wave A drain (trends.py/merge.py int-coerced score fields from 188-05; combined connectors+advanced overlay CI regression test) gating 999.105 Tier 1 (customizable reporting engine), 999.101 (roadmap NOW/NEXT/LATER score-lift re-frame), 999.102 (finding storyline drawer), BACK-51 opportunistic. Sequencing commitment: v5.24 = UAT Coverage Drain. Phase numbering continues at 199. Reminders: phase.complete/milestone.complete/state.* verbs remain UNSAFE — hand-write closes under pre-image + signature-diff.
