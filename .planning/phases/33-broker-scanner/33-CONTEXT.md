...
### Profile gating
- **D-10:** Broker scanning runs in **`standard` and `deep`** profiles only — matches Phase 32 (email) gating. Excluded from `quick`. Adds `cfg.connectors.enable_broker` flag in `quirk/engine/profiles.py:apply_profile()` (parallel to `enable_email`). **[Revised 2026-04-27 post-research: original D-10 cited `cfg.scanners.broker_enabled` namespace which does not exist in `quirk/config.py`. Mirrors Phase 32 actual precedent.]**
...
- **D-15:** `labs/broker/` uses **official images** per REQUIREMENTS BROKER-LAB-01: `rabbitmq:3.12-management`, `redis:7-alpine`, and an official Kafka image (`apache/kafka:3.6` or `confluentinc/cp-kafka:7.5` — planner picks). Weak-TLS configuration uses minimal mounted config files (`rabbitmq.conf` for cipher list + TLS 1.1 minimum, `redis.conf` for tls-protocols / tls-ciphersuites, Kafka `server.properties` for ssl.enabled.protocols + ssl.cipher.suites). **[Revised 2026-04-27 post-research: original D-15 specified Bitnami images, but bitnami/rabbitmq and bitnami/redis expose no env vars for cipher suites or TLS minimum version. Only official images support the cipher-level control needed for Success Criterion 4 (weak-cipher HIGH on ≥2 brokers).]**
...
- Exact AMQP-header byte sequence handling for RABBIT-02 plaintext detection (`b'AMQP\\x00\\x00\\x09\\x01'`) — timeout values, response-frame parsing depth, false-positive suppression. Follow the conservative pattern: 2s timeout, **`len(data) > 0` after sending the AMQP header = positive** (any non-empty TCP response within timeout). **[Revised 2026-04-27 post-research: original spec said "accept any `b'AMQP'` prefix in response" but AMQP 0-9-1 Connection.Start replies are binary METHOD frames, not `b'AMQP'`-prefixed. Literal prefix match would yield false negatives on every real broker.]**
