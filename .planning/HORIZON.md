...
**Last updated:** 2026-08-11 (post-v5.11 ship — full re-evaluation cadence run)
**Current state:** v5.10 Hardware Lifecycle Depth (SHIPPED 2026-08-03, Phases 139–143) and v5.11 Discovery at Scale + Backlog Drain (SHIPPED 2026-08-11, Phases 144–147, tag `v5.11.0`, on PyPI) have both landed since this file was last touched. See PROJECT.md / MILESTONES.md for detail; recaps below, not re-summarized here. **No milestone is currently open** — v5.12 is being shaped. The backlog is the thinnest it has been in the v5.x line: after v5.11 closed the OT/ICS resume-checkpoint gap (DRAIN-01) and the BACnet CVE coverage gap (DRAIN-02), the only substantial unbuilt capability item is **continuous hardware lifecycle monitoring**, which still needs its own research pass. SaaS multi-tenancy stays parked (no business-model signal, unchanged since v5.4).
