...
**Last updated:** 2026-09-07 (post-v5.19 archive — backlog reconciliation audit executed; Open-Item Ledger added below, making this file the single canonical open-item ledger)
...
## Open-Item Ledger — canonical as of 2026-09-10 (v5.21 boundary pass)
...
| **999.104** | P2 | CLI config ↔ dashboard parity — counted field universe is 121 operator-settable fields (not the ~138 PM-era estimate), of which 35 covered + 6 covered-indirectly + 15 intentional-gap + 65 not-yet-covered per the audit; structural parent of 999.96's feature half | Filed 2026-09-08 by PM at v5.21 boundary; tiered shape + feasibility in `.planning/backlog/999.104-cli-config-dashboard-parity/IDEA.md`. **Audited 2026-09-09** by Phase 194 plan 08: `.planning/phases/194-advanced-scan-fields-executive-verdict-phantom-cert-fix/194-PARITY-AUDIT.md`. Tier roll-up: **Tier 1 (visibility) CLOSED**; **Tier 2 (connector parity) CLOSED** (closed 2026-09-10 by Phase 197) — 25/25 `enable_*` flags dispositioned (unchanged) plus 37/37 credential/endpoint/target sub-fields now dashboard-settable via the widened connectors delta overlay (`quirk.dashboard.api.schemas.validate_connectors_overlay` / `_CONNECTOR_DETAIL_KEY_TYPES`), enforced in lockstep at submit, job-YAML merge, and Effective Config preview; see `197-01-SUMMARY.md` (backend widening + lockstep proof), `197-02-SUMMARY.md` (dashboard rendering, 37/37 fields), and `197-03-SUMMARY.md` (PARITY-07 no-leak regression proof + D-14 job-YAML success-criterion-4 proof). The archived `194-PARITY-AUDIT.md` point-in-time record is left unmodified (D-15) — this row is the record of the closure; **Tier 3 (scan-behavior parity) PARTIALLY CLOSED** — AdvancedPanel (194-05) covers 7 of 30 `scan.*`/`timeouts.*`/`retry.*` fields plus 1 covered-indirectly, residue is 11 per-scanner timeouts + 4 concurrency knobs + retry backoff + misc; **Tier 4 (config-file parity) remains explicitly OUT of milestone scope**, per this row's own v5.21 rationale — no server-side `config.yaml` write surface was touched. This row is NOT retired: tiers 2-3 residue is real, named, and left for a future milestone to scope. |
...
| BACK-51 | P3 | migration_planner dual categorization — one targeted check needed | Duality CONFIRMED LIVE as of Phase 189: `quirk/reports/writer.py`'s single `write_reports()` entrypoint (def at line 415) calls both `build_phased_roadmap()` (`writer.py:470`, evidence-driven narrative items with their own `timeframe`, feeding exec report / HTML / DOCX / dashboard roadmap graph) and `categorize_waves()` (`writer.py:843`, severity-bucketed NOW/NEXT/LATER, console-only Migration Waves table) in the same report-generation pass of a single scan — two independently-derived systems answering the same "what should I fix and when" question, both reaching the operator. No code fix attempted in Phase 189: unifying two different data models with different consumers is a small-to-medium redesign, not the targeted check TRIAGE-08 scoped. Priority set to P3, consistent with the neighboring UX-polish rows (BACK-01/03/08) — this is a coherence/duplication issue, not a correctness bug. |
