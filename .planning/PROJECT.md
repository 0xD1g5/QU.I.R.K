...
**v5.5 Distributed Hardening + Stabilization — SHIPPED 2026-05-27** (Phases 113–116, 11 plans, local tag `v5.5.0`). The v5.4 distributed scanner is now production-hardened: each sensor authenticates `POST /api/sensor/push` with its own revocable opaque Bearer token (shared-token model retired); the console auto-merges into one CBOM + score once every non-revoked enrolled sensor checks in, via a failure-isolated FastAPI BackgroundTask with a config toggle and two trigger conditions; the live-UAT defect sweep made the distributed lab re-runnable without teardown and added a weak-TLS segment-b target so the per-segment filter is exercisable end-to-end; and an evidence-backed PyInstaller spike returned GO-conditional for a v5.6 Windows frozen-EXE build. Milestone audit: 13/13 requirements satisfied, 0 blockers, cross-phase integration clean. Full record: `.planning/MILESTONES.md` + `.planning/milestones/v5.5-ROADMAP.md`.

**Next: v5.6 (planning)** — continues from Phase 117. Seeds from the v5.5 close: the full Windows frozen-binary build (`--onedir` EXE + Scheduled Task host + installer, gated on confirming the WINPKG-01 spike's GO via the live windows-latest CI build), and the public-repo cutover + required-status-check enforcement. The Hardware Compatibility & Lifecycle Remediation initiative (HWCOMPAT-01..06) and the SaaS multi-tenancy question remain in the backlog / parked. Source of truth for forward outlook: `.planning/HORIZON.md`.

**Deferred at close:** 2 human-UAT (UAT-114-03 operators-guide §8.9 visual review; UAT-116-02 live windows-latest spike CI build — confirms the WINPKG-01 GO on push) + cosmetic empty SUMMARY frontmatter. The branch is unpushed (~100+ commits ahead of origin); pushing also triggers the live CI build.
...
**Target features:**
- **Architecture document + data-model design (ANCHOR — Phase 1, folds in 999.58):** the sensor/console contract, enrollment/auth model, and the `sensor_id`/segment dimension on `CryptoEndpoint` (same RFC1918 IP can legitimately exist in two segments). Design input for everything downstream; no code until this lands.
...
- **Cross-platform sensors — Windows in scope:** the sensor (ideally the full tool) must run on Windows, not just Linux. Windows runtime host (Service/Scheduled Task, not cron/systemd), packaging for Python-less locked-down boxes (frozen exe / container), a POSIX-ism audit, and a Windows CI smoke-test validation path. **Sizing risk** — the arch-doc phase decides whether full Windows-sensor support lands in v5.4 or splits to a v5.5 fast-follow; v5.4 floor is keeping the sensor↔console contract OS-agnostic.
- **Stabilization tail:** 999.59 (operators-guide all-configurations coverage), extract duplicated `_NoRedirectHandler` → `quirk/util/no_redirect.py`, residual dep hygiene.

**Key context:**
- **SaaS multi-tenancy stays PARKED** — distributed on-prem is a network-topology / engagement-completeness necessity (segmented enterprise nets a single host can't reach), NOT a business-model bet. The "multi-segment customer ask" gate that held 999.22 is now satisfied (PM review 2026-05-25); SaaS remains gated on a business-model signal that does not yet exist.
- **Single-tenant only; architecture doc first (Phase 1) before any code.** This is the largest architectural change in the project's history (new service role + data-model change + cross-platform sensors) — research-first milestone.
- **Groundwork is freshest now:** v5.3 just built the console-side primitives (token-auth ingestion, outbound-push, delivery-audit, SSRF discipline); a sensor→console push is their mirror image.
