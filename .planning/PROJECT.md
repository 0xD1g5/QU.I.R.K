...
## Current Milestone: v5.5 Distributed Hardening + Stabilization
...
**v5.5 Distributed Hardening + Stabilization is now OPEN** (see Current Milestone above), continuing from Phase 113. It pulls in the v5.4 carry-forward — per-sensor token auth + revocation (TD-1), automatic merge-trigger (106 D-06) — plus the 999.85–89 live-UAT defect sweep; Windows packaging (106 D-05) enters as a spike-only sizing phase (full build deferred to v5.6). Public-repo cutover stays deferred.

**Phase 113 — Per-Sensor Authentication — COMPLETE 2026-05-27** (3 plans, verification PASSED 4/4, AUTH-01..04). Replaced the v5.4 shared-token model with per-sensor opaque Bearer tokens: a new `require_sensor_auth` dependency (SHA-256 lookup, `hmac.compare_digest`, `revoked_at` enforcement, audit rows on every branch) protects a split-out `sensor_push_router`; `revoke-sensor` CLI cuts off one compromised sensor without affecting others; enrollment stores only the hash and shows the raw token once; operators-guide §8.1.1 documents backward-compatible migration. Code review: 0 Critical, 4 Warning (WR-01 fail-closed fallback, WR-02 token-authoritative response id, WR-03 audit-commit rollback hygiene, WR-04 idempotent revoke) carried as non-blocking follow-ups. Next: Phase 114 (Automatic Merge Trigger).
