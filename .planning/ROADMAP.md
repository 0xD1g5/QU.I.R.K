...
| BACK-89 | **[P2 — UI] Executive Summary score vs severity-bar consistency** | Dashboard / data-truth consistency | Discovered during v4.10 UAT-85-08 (release dry-run, 2026-05-21) while capturing the marketing hero screenshot against the `tls-cert-defects` chaos lab profile. The Executive Summary view renders `Overall Readiness: 100` (with "Medium Confidence" annotation underneath) alongside a Severity Breakdown bar chart that shows non-zero **CRITICAL** and **HIGH** bars from the same scan (34 endpoints, 6 findings). The two indicators appear to read different metrics and create a reads-as-contradictory impression for a first-time viewer: the gauge implies "all good," the bars imply "real findings exist." Subscore gauges in the same row (Hygiene 25, Modern TLS 25, Identity 24, Agility 3, Data at Rest 25, Data in Motion 20) sum to ~122, which doesnt obviously normalize to the 100 Overall Readiness either — Agility=3 in particular suggests a poor sub-dimension that should drag overall readiness below 100. **Investigation needed:** (a) What does the 100 Overall Readiness gauge actually represent — is it raw `compute_overall_score()` output, a coverage percentage, a confidence-modulated number, or a stale cached value from a prior empty scan? (b) Why does "Medium Confidence" annotation appear next to a maxed-out 100? (c) Should the Overall Readiness be deducted by per-severity-bar finding counts so the two indicators tell the same story? (d) If the math is correct but the UX is misleading, redesign the gauge label/annotation (e.g., split "Coverage 100% (Medium Confidence)" from "Risk Score 67/100 — CRITICAL findings present"). **Why P2:** Not a security-correctness bug — the underlying CBOM + findings are accurate — but its the first thing a security consultant will question in a sales demo. The Certificate Inventory view (used as the v4.10 UAT-85-08 secondary screenshot `docs/images/dashboard-hero-2.png`) shows the findings unambiguously, which proves the data is correct upstream of the Executive Summary aggregation. **Pointers:** `quirk/api/score.py::compute_overall_score`, `quirk/dashboard/api/routes/scan.py` (Executive Summary endpoint), React Executive page gauge component. **Source:** UAT-85-08 release dry-run; screenshots at `docs/images/dashboard-hero-1.png` (Executive Summary, current state) and `docs/images/dashboard-hero-2.png` (Certificate Inventory, ground-truth findings). Phase dir: `.planning/backlog/999.82-executive-summary-score-vs-severity-consistency/` |
| BACK-90 | **[P1 — RELENG] Release UAT automation (CI workflows + scripts replacing manual UAT-85-09..12)** | Release engineering / continuous gates | v4.10 shipped with manual UAT-85-09 (asciinema demo), UAT-85-10 (clean-machine end-to-end), and UAT-85-12 (Homebrew tap bootstrap) deferred. UAT-85-11 (v4.10.0 tag-push) ran manually. All four are automatable; the manual versions exist only because the automation does not yet exist. Promoting them to CI converts release-time risk into PR-time risk, which is dramatically cheaper to fix and runs continuously rather than once per release. **Scope (4 deliverables):** **(1) `scripts/record-demo.sh`** — wraps `asciinema rec --command='quirk --config demo/config.yaml'` against a curated demo config pointing at the chaos lab `tls-cert-defects` profile; on successful record, prompts the user to review, then runs `asciinema upload` (requires one-time `asciinema auth`); on upload success, sed-substitutes the resulting URL into README.md replacing the `<asciinema-link-here>` placeholder. Quality-of-recording is the limiting factor; the script makes the mechanics unattended. **(2) `.github/workflows/release-quickstart-smoke.yml`** — GitHub Actions workflow running on `macos-latest` (arm64 since 2024). On every PR touching README.md, docs/getting-started.md, pyproject.toml, or run_scan.py: runs the 3-step quickstart verbatim from docs/getting-started.md in a clean venv (`python3 -m venv .venv && source .venv/bin/activate && pip install quirk-scanner[all] && quirk init && quirk --config config.yaml --targets-file <fixture>`). Asserts exit 0, asserts CBOM artifact lands at the documented path. Replaces manual UAT-85-10. **(3) `.github/workflows/release-smoke.yml`** — fires *after* release.yml + release-container.yml succeed (uses `workflow_run` trigger with `types: [completed]` and `branches: [main]`); verifies PyPI listing (`pip index versions quirk-scanner` shows the just-published version), Sigstore attestation (`gh attestation verify --owner 0xD1g5 dist/quirk-scanner-X.Y.Z-*.whl`), GHCR multi-arch pullability (`docker buildx imagetools inspect ghcr.io/0xd1g5/quirk:vX.Y.Z` shows both `linux/amd64` and `linux/arm64`). Replaces manual UAT-85-11 post-pipeline verification. **(4) `scripts/bootstrap-tap.sh`** — for Homebrew tap mechanics: fetches sdist from PyPI, computes sha256, substitutes placeholders in `Formula/quirk.rb`, commits to the `0xD1g5/homebrew-quirk` tap repo (requires `gh repo create` and SSH/HTTPS auth one-time setup). The `brew install 0xD1g5/quirk/quirk` verification on macOS arm64 is run via a separate workflow `.github/workflows/tap-install-smoke.yml` that runs on `macos-latest` after the tap repo updates. Replaces the mechanical portion of manual UAT-85-12 (the one-time GitHub repo creation stays manual). **Why P1:** Without this, every future v4.X release relives the same 4 manual UAT items, and any drift in the install/release surfaces only at release time. With this in place, v4.11+ ships through the same automated gates that catch UAT-85-09..12 issues at PR time. **Source:** v4.10 release dry-run, Path C decision 2026-05-21. Phase dir: `.planning/backlog/999.83-release-uat-automation/` |
| BACK-89 | **[P2 — UI] Executive Summary score vs severity-bar consistency** | Dashboard / data-truth consistency | Discovered during v4.10 UAT-85-08 (release dry-run, 2026-05-21) while capturing the marketing hero screenshot against the `tls-cert-defects` chaos lab profile. The Executive Summary view renders `Overall Readiness: 100` (with "Medium Confidence" annotation underneath) alongside a Severity Breakdown bar chart that shows non-zero **CRITICAL** and **HIGH** bars from the same scan (34 endpoints, 6 findings). The two indicators appear to read different metrics and create a reads-as-contradictory impression for a first-time viewer: the gauge implies "all good," the bars imply "real findings exist." Subscore gauges in the same row (Hygiene 25, Modern TLS 25, Identity 24, Agility 3, Data at Rest 25, Data in Motion 20) sum to ~122, which doesnt obviously normalize to the 100 Overall Readiness either — Agility=3 in particular suggests a poor sub-dimension that should drag overall readiness below 100. **Investigation needed:** (a) What does the 100 Overall Readiness gauge actually represent — is it raw `compute_overall_score()` output, a coverage percentage, a confidence-modulated number, or a stale cached value from a prior empty scan? (b) Why does "Medium Confidence" annotation appear next to a maxed-out 100? (c) Should the Overall Readiness be deducted by per-severity-bar finding counts so the two indicators tell the same story? (d) If the math is correct but the UX is misleading, redesign the gauge label/annotation (e.g., split "Coverage 100% (Medium Confidence)" from "Risk Score 67/100 — CRITICAL findings present"). **Why P2:** Not a security-correctness bug — the underlying CBOM + findings are accurate — but its the first thing a security consultant will question in a sales demo. The Certificate Inventory view (used as the v4.10 UAT-85-08 secondary screenshot `docs/images/dashboard-hero-2.png`) shows the findings unambiguously, which proves the data is correct upstream of the Executive Summary aggregation. **Pointers:** `quirk/api/score.py::compute_overall_score`, `quirk/dashboard/api/routes/scan.py` (Executive Summary endpoint), React Executive page gauge component. **Source:** UAT-85-08 release dry-run; screenshots at `docs/images/dashboard-hero-1.png` (Executive Summary, current state) and `docs/images/dashboard-hero-2.png` (Certificate Inventory, ground-truth findings). Phase dir: `.planning/backlog/999.82-executive-summary-score-vs-severity-consistency/` |
| BACK-A11Y-01 | **[P2 — UI/a11y] Regenerate dashboard a11y baselines + fix color-contrast / button-name violations** | dashboard-quality CI a11y gate | Discovered during Phase 87 DEP-01 (2026-05-22). With the Node 24 bump + the eslint-gate fix landed, the `dashboard-quality` workflow reached its `Run axe + console sweep` step **for the first time ever in CI** — lint had always failed first and skipped it. The sweep is RED across **all 11 dashboard pages** (root, findings, identity, motion, data-at-rest, certificates, cbom, roadmap, trends, qramm, qramm-assessment) on two distinct problems: **(a) stale baselines** — `src/dashboard/tests/a11y/baseline-*.json` record violations on selectors (e.g. `.h-9`) that no longer match the UI after many phases of evolution, while the live render flags different selectors (`.bg-accent`, `.text-muted-foreground`, sortable `th[aria-sort]`, severity badges); and **(b) genuine WCAG-AA violations** — `color-contrast` on muted-foreground text / accent backgrounds / severity badges / table headers, and `button-name` on radix dropdown trigger buttons that lack discernible text. **Scope:** (1) audit all 11 pages; (2) fix real contrast issues via design-token adjustments (likely `--muted-foreground` + accent/badge hsl values to reach 4.5:1) and add `aria-label`/visible text to icon-only radix triggers; (3) regenerate `baseline-*.json` **inside CI** against the corrected UI so the gate reflects real state and stays meaningful. **Why deferred from 87:** orthogonal to the Node runtime (fails identically on Node 20); a UI/design workstream, not dependency hygiene — Node 24 deadline-clearing was verified through Setup Node + Install + Build + Lint. **Pointers:** `src/dashboard/tests/a11y/run-a11y.mjs`, `baseline-*.json`, `src/dashboard/src/index.css` (design tokens), shadcn dropdown/table components. **Source:** Phase 87 PR #4 run 26297453788. |
...
## v4.9 Audit Depth — Phases 69–77 — SHIPPED 2026-05-15
...
## v4.10 Launch Readiness — Coverage, Hardening, Release Engineering — Phases 78–85 — SHIPPED 2026-05-21

Archived to: [`.planning/milestones/v4.10-ROADMAP.md`](milestones/v4.10-ROADMAP.md). 8 phases, 31 plans, 52/52 requirements, 5 deferred human UAT for release dry-run.
...
## v4.10.1 Scoring Correctness Hotfix — Phase 86 — SHIPPED 2026-05-22

Archived to: [`.planning/milestones/v4.10.1-ROADMAP.md`](milestones/v4.10.1-ROADMAP.md). 1 phase, 3 plans, 8/8 requirements. Normalized overall readiness (`int(round(sum/1.5))`) + ScoreGauge `maxValue` color fix + version bump to 4.10.1. EVIDENCE-TALLY-01 / RENDER-CLI-01 / RENDER-PDF-01 deferred to v5.0 Phase 01 (Stabilization).
...
| 91. Code Cleanup + Bookkeeping | 1/3 | In Progress|  |
