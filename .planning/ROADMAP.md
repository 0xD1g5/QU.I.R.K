...
- [x] **Phase 169: UAT Record Drain — Series ~100–163 + Enforcement** - The remaining unrecorded UAT cases are drained to zero, and a standing check prevents the corpus from silently re-accumulating undispositioned cases. Plans executed (2026-08-28; 8/8 plans done — 78/78 series-101-163 cases dispositioned, full 666-case document + 377-row ledger 100% dispositioned; `tests/test_uat_zero_undispositioned_gate.py` standing gate live and documented in all four D-07 locations; full-suite baseline held at 1 pre-existing failure, zero fatal signals, 3647 passing); verified 2026-08-28 — 169-VERIFICATION.md status: passed, 3/3 must-haves; gate proven load-bearing by mutating a scratch corpus (UAT-140-01 flagged by ID+line); 6 DEFERRED substitutes re-run independently; human checkpoint approved.
- [x] **Phase 170: Traceability, Documentation & Runbook** - The changelog, archive status headers, requirement-to-test linkage, planning-summary cross-references, and CLAUDE.md's staleness runbook are all brought into agreement with what actually shipped and what CI actually gates.
- [x] **Phase 171: Resume UX Tail** - Resuming an already-complete scan short-circuits cleanly, and `--list-resumable` shows the correct target for every run type.
...
- [x] 165-03-PLAN.md — Exact dependency pins, vitest include-glob widening, CI npm test step (D-04)
- [x] 165-04-PLAN.md — Contrast token fixes, severity-token promotion, ScanSelector aria-label (D-08..D-10, D-12)
...
- [x] 165-06-PLAN.md — ACCEPTED-VIOLATIONS.md generator and its byte-compare freshness test (D-05/D-07)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 165-07-PLAN.md — Regenerate all baselines against the fixed UI, write justifications, green the gates (D-11)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 165-08-PLAN.md — Docs updates, Obsidian vault sync, UAT-SERIES.md, phase note
...
**Success Criteria** (what must be TRUE):
  ...
  2. `uat_runner.py` parses all XML through `quirk.util.xml_safe.parse_safely()`, the Phase 87 /
     DEP-02 hardened lxml chokepoint, rather than stdlib `ElementTree`; no
     XXE/billion-laughs-vulnerable parse path remains in that module, and an AST-based gate in
     `tests/test_xml_safe.py` forward-locks the file against a stdlib-xml or `defusedxml`
     regression. **Corrected 2026-08-27 during Phase 166 (166-02):** the original wording here
     said to migrate to `defusedxml`, framed as matching a v5.0 SAML migration — that premise was
     factually backwards. Phase 87 / DEP-02 migrated *away from* `defusedxml` to the lxml
     chokepoint, and two CI gates (`tests/test_packaging.py::test_defusedxml_not_in_core_deps`,
     `tests/test_xml_safe.py::test_no_defusedxml_import_in_quirk`) actively forbid reintroducing
     it. Phase 87 / DEP-02 is the real precedent, not the v5.0 SAML path.
...
- [x] 171-01-PLAN.md — RESUME-05: short-circuit resume of an already-complete scan (D-01)
...
- [x] 171-03-PLAN.md — Phase close: full-suite proof, Series 171 UAT entry, REQUIREMENTS.md, Obsidian sync, human checkpoint
...
| 171. Resume UX Tail | 3/3 | Complete   | 2026-08-28 |
