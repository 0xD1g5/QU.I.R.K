...
| scanners-protocol/IN-01 | INFO | tls_capabilities downgrade SSLContext warrants comment | Phase 77 (77-01 / D-01) | [x] closed — closed by Phase 77 (77-01 / D-01 / IN-01): 3-line `# WHY:` comment block inserted above `_try_handshake` body at quirk/scanner/tls_capabilities.py:52-55; phrase `legacy-server posture` cites audit row. RESEARCH C-8 adjudication: function probes deprecated TLS bands as inventory (no single "downgrade site"). Test: tests/test_tls_capabilities_comment.py. |
| scanners-protocol/IN-02 | INFO | DNSSEC_ALG_MAP missing reserved algorithms 9, 11 | Phase 77 (77-01 / D-02) | [x] closed — closed by Phase 77 (77-01 / D-02 / IN-02): added `9: ("Reserved", "HIGH")` and `11: ("Reserved", "HIGH")` per IANA DNS Security Algorithm Numbers / RFC 8624 (quirk/scanner/dnssec_scanner.py:51, 53). RESEARCH C-9 adjudication: alg 9 is Reserved (NOT RSASHA1-NSEC3-SHA1, which is alg 7 already present). Tests: tests/test_dnssec_scanner_reserved_algs.py + tests/test_dnssec_scanner.py broadened 12→14 keys. |
| scanners-protocol/IN-03 | INFO | SHA1_INDICATORS substring match too loose | Phase 77 (77-01 / D-03) | [x] closed — closed by Phase 77 (77-01 / D-03 / IN-03): `_is_sha1_uri` now delegates to `quirk.assessment.migration_advisor._matches('SHA1', uri)` (Phase 74 word-boundary helper) at quirk/scanner/saml_scanner.py. CANONICAL_ALG_SYNONYMS covers SHA1/SHA-1; `SHA1024` no longer mis-detected. Test: tests/test_saml_scanner_sha1_word_boundary.py. |
| scanners-protocol/IN-04 | INFO | fingerprint _http_probe_plain sends Host: localhost | Phase 77 (77-01 / D-04) | [x] closed — closed by Phase 77 (77-01 / D-04 / IN-04): HTTP/1.0 request `Host:` header now sends target hostname (`Host: {host}`) instead of literal `localhost` at quirk/scanner/fingerprint.py:111-117; ASCII-safe encoded. Test: tests/test_fingerprint_host_header.py. |
| scanners-protocol/IN-05 | INFO | _is_pfs / _is_weak duplicated across email/broker/tls | Phase 77 (77-01 / D-05) | [x] closed — closed by Phase 77 (77-01 / D-05 / IN-05): two public helpers `is_pfs_cipher` + `is_weak_cipher_classification` added at quirk/util/weak_crypto.py:72-105; local `_is_pfs`/`_is_weak` deleted from broker_scanner.py, email_scanner.py, tls_scanner.py; 6 call sites rerouted. Test: tests/test_weak_crypto_pfs_weak_helpers.py. |
| scanners-protocol/IN-06 | INFO | kerberos _derive_realm IPv4 detection is fragile | Phase 77 (77-01 / D-06) | [x] closed — closed by Phase 77 (77-01 / D-06 / IN-06): `_derive_realm` replaced dotted-quad heuristic with `ipaddress.ip_address(stripped)` strict parser in try/except ValueError at quirk/scanner/kerberos_scanner.py:18 (import) + :51-71 (body). Now correctly classifies IPv6 literals. Test: tests/test_kerberos_scanner_realm_ipaddress.py. |

### Scanners — Cloud + Engine

| Finding ID | Severity | Title | Closed-By | Status |
|---|---|---|---|---|
| scanners-cloud/CR-01 | BLOCKER | migration_planner.py is a stub — does not implement scoring | — | [ ] wont-fix — `quirk/cbom/migration_planner.py` is a 16-line stub; v4.x decision is that migration-planning intelligence is the consumer's responsibility (out-of-scope for cryptographic inventory scope). Re-evaluate in v5.0 if a typed migration pipeline is needed. Inline-rationale added by Phase 77 LEDGER-01 (D-30 / 77-05). |
...
| scanners-cloud/CR-03 | BLOCKER | K8s scan_k8s_targets calls _scan_aks_encryption with None cred | — | [ ] deferred-v4.9 — Phase 29 documented the credentialed AKS path; the None-cred fallback is exercised in production only when running unauthenticated against an AKS cluster; the existing graceful-degradation path returns an empty list (correct). v4.9 will add an explicit `K8S-04` log for the None-cred branch. Inline-rationale added by Phase 77 LEDGER-01 (D-30 / 77-05). |
...
| cbom-intel-reports/IN-01 | INFO | Hardcoded PLATFORM_VERSION = 4.4.0 duplicated across modules | Phase 77 (77-02 / D-07) | [x] closed — closed by Phase 77 (77-02 / D-07 / IN-01): two literal duplicates at quirk/cbom/builder.py:128 and quirk/reports/writer.py:23 replaced with `from quirk import __version__ as PLATFORM_VERSION`. Single source: quirk/__init__.py. Test: tests/test_platform_version_single_source.py. |
| cbom-intel-reports/IN-02 | INFO | _extract_ssh_algorithms swallows JSONDecodeError silently | Phase 77 (77-02 / D-08) | [x] closed — closed by Phase 77 (77-02 / D-08 / IN-02): `_extract_ssh_algorithms` JSONDecodeError now logged via `logger.warning("Failed to parse SSH algorithms JSON: %s", safe_str(e))` (Phase 59 safe_str reuse); TypeError/ValueError continue silently (non-string shape, not malformed content). Test: tests/test_cbom_builder_ssh_json_error.py. |
| cbom-intel-reports/IN-03 | INFO | Trend analysis fetches all endpoints into memory per session | Phase 77 (77-02 / D-09) | [x] closed — closed by Phase 77 (77-02 / D-09 / IN-03): `_fetch_session_endpoints` in quirk/intelligence/trends.py switched from `.all()` to `.yield_per(1000)` + `list(streamed)`; memory ceiling now bounded by chunk size rather than full row count. Test: tests/test_trends_yield_per.py. |
| cbom-intel-reports/IN-04 | INFO | evidence.py _PROTOCOL_KEYS missing CONTAINER/SOURCE/AWS/etc. | Phase 77 (77-02 / D-10) | [x] closed — closed by Phase 77 (77-02 / D-10 / IN-04): added 6 keys to `_PROTOCOL_KEYS` in quirk/intelligence/evidence.py: CONTAINER, SOURCE, AWS, AZURE, GCP, CLOUD_SQL. RESEARCH C-10: KUBERNETES + VAULT already present pre-Phase-77. Test: tests/test_evidence_protocol_keys.py. |
| cbom-intel-reports/IN-05 | INFO | Roadmap baseline governance item only when len < min_items | Phase 77 (77-02 / D-11) | [x] closed — closed by Phase 77 (77-02 / D-11 / IN-05): post-loop unconditional governance check added to `build_phased_roadmap` in quirk/intelligence/roadmap.py — "Establish crypto governance review" item now appears OR is added regardless of `min_items` short-circuit. Test: tests/test_roadmap_baseline_governance.py. |
| cbom-intel-reports/IN-06 | INFO | executive truncates Migration Paths at 10 with no indicator | Phase 77 (77-02 / D-12) | [x] closed — closed by Phase 77 (77-02 / D-12 / IN-06): `build_exec_markdown` in quirk/reports/executive.py now appends `- ... and {remaining} more (see full report)` when `len(recs) > 10`. Migration Paths section self-documents truncation. Test: tests/test_executive_truncation_indicator.py. |
| cbom-intel-reports/IN-07 | INFO | html_renderer roadmap_section timeframe comparison dead branch | Phase 77 (77-02 / D-13) | [x] closed — audit-flip-only per Phase 77 (77-02 / D-13 / IN-07): RESEARCH C-7 / Pitfall 9 verification — mutation test in tests/test_html_renderer_roadmap_section.py proves both `r.get("timeframe") == tf` and `r.get("phase") == tf` branches reachable. No dead code. Inline `# Phase 77 D-13 / cbom-intel-reports/IN-07: C-7 verification` comment added above the filter at quirk/reports/html_renderer.py. |
| cbom-intel-reports/IN-08 | INFO | writer hosts_count set falsy hosts collapse to single "" | Phase 77 (77-02 / D-14) | [x] closed — closed by Phase 77 (77-02 / D-14 / IN-08): new module-level helper `_unique_hosts(hosts) -> set` in quirk/reports/writer.py filters None/empty-string entries before set construction; `hosts_count` now calls through this helper. Scans with endpoints missing host no longer over-count. Test: tests/test_writer_hosts_count_filters_falsy.py. |
| cbom-intel-reports/IN-09 | INFO | Schema dataclass IntelligenceReport defined but unused | Phase 77 (77-02 / D-15 PIVOT) | [x] closed — closed by Phase 77 (77-02 / D-15 PIVOT / IN-09): user-directed PIVOT from cascade-delete to preserve-with-docstring + CI-guard. RESEARCH C-1 surfaced live importers (`tests/test_intelligence_schema.py`). Class preserved with "Typed return shape for the intelligence pipeline aggregate output" docstring at quirk/intelligence/schema.py; package export preserved; new tests/test_intelligence_public_api.py enforces the typed-return-shape contract + importer-field-exercise contract. |
...
| api-cli-core/WR-10 | WARNING | interactive _prompt_int infinite loop on EOF | Phase 75 | [x] closed |
| api-cli-core/WR-11 | WARNING | interactive exposure default silently used when input not 1/2/3 | Phase 75 | [x] closed |
| api-cli-core/WR-12 | WARNING | setattr enable_nmap injects undeclared dataclass attribute | Phase 75 | [x] closed |
| api-cli-core/WR-13 | WARNING | validate.py artifact list missing intelligence-{stamp}.json | Phase 75 | [x] closed |
| api-cli-core/WR-14 | WARNING | qramm_cmd env override has no try/except on malformed input | Phase 75 | [x] closed |
| api-cli-core/WR-15 | WARNING | routes/scan reads QUIRK_OUTPUT_DIR from env into Path read | Phase 75 | [x] closed |
| api-cli-core/WR-16 | WARNING | parse_target_tokens does not validate hostnames | Phase 75 | [x] closed |
| api-cli-core/WR-17 | WARNING | routes/qramm list_questions fragile on QRAMM_QUESTIONS schema drift | Phase 75 | [x] closed |
| api-cli-core/IN-01 | INFO | Dict[str, Any] type erasure on QRAMM endpoints | Phase 77 (77-03 / D-16) | [x] closed — closed by Phase 77 (77-03 / D-16 / IN-01): `QrammScoreResponse(BaseModel)` Pydantic model added with 5 canonical fields (session_id, overall, maturity, dimensions, profile_multiplier); `@router.post("/qramm/sessions/{session_id}/score", response_model=QrammScoreResponse)` annotation at quirk/dashboard/api/routes/qramm.py. Per-dim full TypedDict narrowing deferred to v5.0 (Discretion). Test: tests/test_qramm_routes_typed_response.py. |
| api-cli-core/IN-02 | INFO | _FACES banner has \- ambiguous escape (comment misleading) | Phase 77 (77-03 / D-17) | [x] closed — closed by Phase 77 (77-03 / D-17 / IN-02): comment-only fix per RESEARCH C-5. `_FACES = (r"…")` is a raw string so `\-` is literal text; only the comment was misleading. Rewrote misleading comment at quirk/cli/banner.py to state the raw-string fact and cite audit row. Code byte-for-byte unchanged. Test: tests/test_banner_comment_corrected.py. |
| api-cli-core/IN-03 | INFO | interactive timezone fallback to UTC string vs IANA name | — | [ ] wont-fix — site not present at HEAD per Phase 77 (77-03 / D-18 / IN-03): RESEARCH C-11 / Pitfall 6 / Q2 — Wave-0 grep across quirk/cli/, quirk/dashboard/api/, quirk/qramm/ for tzlocal/UTC-fallback/datetime.tzinfo/pytz returned zero matches at HEAD cf2417a. Audit row stale or pre-fixed by an earlier phase. Forward-protected by tests/test_audit_ledger_zero_open.py invariant. |
| api-cli-core/IN-04 | INFO | Magic numbers for QRAMM clamp 0.8 / 1.5 / 0.10 / 0.20 | Phase 77 (77-03 / D-19) | [x] closed — closed by Phase 77 (77-03 / D-19 / IN-04): module-level constants MULTIPLIER_MIN=0.8, MULTIPLIER_MAX=1.5, MULTIPLIER_LOW_STEP=0.10, MULTIPLIER_HIGH_STEP=0.20 added to quirk/dashboard/api/routes/qramm.py (per RESEARCH C-2, NOT scoring.py); 4 literal occurrences replaced across `_SENSITIVITY_DELTA` map, `_compute_multiplier` clamp, and score-endpoint server-side validation. AST gate confirms zero stray literals. Test: tests/test_qramm_multiplier_constants.py. |
| api-cli-core/IN-05 | INFO | app.py closure captures via default argument missing | Phase 77 (77-03 / D-20) | [x] closed — audit-flip-only per Phase 77 (77-03 / D-20 / IN-05): audit-row prose ("_make_handler(fp, mt) is the canonical factory pattern — implementation is correct. No-op.") confirmed at quirk/dashboard/api/app.py:124-127 per RESEARCH C-6. Existing `_make_handler(fp, mt)` factory already captures parameters by value at call time. No code change. |
| api-cli-core/IN-06 | INFO | db.py _ensure_*_columns helpers collapse to one helper | Phase 77 (77-03 / D-21) | [x] closed — closed by Phase 77 (77-03 / D-21 / IN-06): generic `_ensure_columns(engine, table, expected)` SQLite ALTER-TABLE-IF-MISSING helper added at quirk/db.py with `_SAFE_COL_RE` + `_SAFE_COL_TYPE_RE` defenses preserved; 8 per-feature helpers removed and their column lists became 8 module-level tuple constants. 5 table-creating helpers preserved (different pattern). Net ~56% LOC reduction. Test: tests/test_db_ensure_columns_generic.py. |
| api-cli-core/IN-07 | INFO | targets.py projected_probe_count materializes /8 host list | — | [ ] wont-fix — Phase 65 Risks #4 documented decision per Phase 77 (77-03 / D-22 / IN-07): `.num_addresses` would re-introduce IPv4 /24 off-by-2 (see comment at quirk/util/targets.py:218-221). Re-evaluate only if Phase 65 Risks #4 is itself reversed. RESEARCH C-3 / Pitfall 3 / Q1 verified the inline comment at HEAD. Threat T-77-03-04 accepted this disposition. |
...
| react-frontend/IN-01 | INFO | qramm-assessment comment says 5-tab but renders 6 tabs | Phase 77 (77-04 / D-23) | [x] closed — closed by Phase 77 (77-04 / D-23 / IN-01): comment at src/dashboard/src/pages/qramm-assessment.tsx:246 rewritten from `{/* 5-tab assessment layout */}` to `{/* 6-tab assessment layout: CVI, SGRM, DPE, ITR, Scorecard, Compliance Map — closes react-frontend/IN-01 (D-23) */}`. Comment-only; zero behavior change. Test: src/dashboard/src/pages/__tests__/qramm-assessment-tab-comment.test.tsx. |
| react-frontend/IN-02 | INFO | cbom/roadmap try/catch swallows extension registration error | Phase 77 (77-04 / D-24) | [x] closed — closed by Phase 77 (77-04 / D-24 / IN-02): both cbom.tsx:19-23 (coseBilkent) and roadmap.tsx:11-16 (dagre) now log via `console.error` AND re-throw when error message does NOT match `/already/i` — RESEARCH C-12 Pattern 8 HMR-safe re-throw. Loud on genuine failures, silent on Vite HMR re-registration. Test: src/dashboard/src/pages/__tests__/cbom-cytoscape-catch.test.tsx. |
| react-frontend/IN-03 | INFO | findings/identity recreate columns array on every render | Phase 77 (77-04 / D-25) | [x] closed — closed by Phase 77 (77-04 / D-25 / IN-03): findings.tsx::columns and identity.tsx::columns wrapped in `useMemo<ColumnDef<FindingItem \| IdentityFinding>[]>(() => [...], [])`. Empty deps verified safe: cell callbacks close only over row.original and module-level constants. Test: src/dashboard/src/pages/__tests__/findings-columns-memo.test.tsx. |
| react-frontend/IN-04 | INFO | useQRAMMSession seededRef not reset on New Assessment flow | Phase 77 (77-04 / D-26) | [x] closed — closed by Phase 77 (77-04 / D-26 / IN-04): `resetSession = useCallback(() => { seededRef.current = null }, [])` added to useQRAMMSession.ts and exposed via the hook's return + `UseQRAMMSessionResult` interface; qramm-assessment.tsx::handleNewAssessment now invokes resetSession after archive succeeds. Test: src/dashboard/src/hooks/__tests__/useQRAMMSession-reset.test.ts. |
| react-frontend/IN-05 | INFO | cbom compByAlg lookups use [0] for representative — drops variance | Phase 77 (77-04 / D-27) | [x] closed — closed by Phase 77 (77-04 / D-27 / IN-05): exported `firstNonZeroComp<T>(comps)` helper added to cbom.tsx; both `compByAlg[d.label]?.[0]` (line 285) and `compByAlg[alg]?.[0]` (line 390) call sites replaced. Generic relaxed to `<T>` with runtime count-probe (CbomComponent has no count field today — falls through to comps[0], preserving pre-D-27 semantic bit-for-bit; future-proof). Test: src/dashboard/src/pages/__tests__/cbom-compByAlg-statistic.test.tsx. |
| react-frontend/IN-06 | INFO | print createElement style injection works but non-standard React | Phase 77 (77-04 / D-28) | [x] closed — closed by Phase 77 (77-04 / D-28 / IN-06): `createElement("style", null, PRINT_CSS)` replaced with standard JSX `<style>{PRINT_CSS}</style>` in print.tsx (RESEARCH Pattern 7); `createElement` import removed entirely (no other call sites). Phase 62 BR-05 cleanup + Phase 76 D-03 sentinel guard untouched. Test: src/dashboard/src/pages/__tests__/print-no-createElement.test.tsx. |
| react-frontend/IN-07 | INFO | useScanData does not propagate actual fetch URL into errors | Phase 77 (77-04 / D-29) | [x] closed — closed by Phase 77 (77-04 / D-29 / IN-07): fetch URL hoisted out of try-block in useScanData.ts (RESEARCH Pitfall 8) so it is in scope for both non-ok branch (`Failed to fetch ${url}: ${resp.status} ${resp.statusText}`) and catch handler (`Failed to fetch ${url}: ${err.message}`). 401/403/404/429 typed-error paths preserved unchanged. Test: src/dashboard/src/hooks/__tests__/useScanData-error-url.test.ts. |
