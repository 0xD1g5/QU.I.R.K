    ...
    try:
        ...
        with socket.create_connection((host, port), timeout=timeout) as sock:
            ...
            with ctx.wrap_socket(sock, server_hostname=server_hostname) as ssock:
                ...
                # ✅ Avoid cryptography eager fallback warnings
                ...
                if logger:
                    logger.v(f"✅ TLS {host}:{port} {ep.tls_version} {ep.cert_pubkey_alg or ''}{ep.cert_pubkey_size or ''} {ep.cert_subject or ''}")
...
def scan_tls_targets(
    ...
    if logger:
        logger.stamp(f"Starting TLS scans: {len(targets)} targets (workers={cfg.scan.concurrency})")

    with ThreadPoolExecutor(max_workers=cfg.scan.concurrency) as ex:
        futures = [
            ex.submit(scan_one, host, port, cfg.scan.timeout_seconds, cfg.scan.include_sni, logger)
