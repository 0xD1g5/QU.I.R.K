...
| `test_posture_scorefix125.py::test_gcp_kms_403_emits_scan_error` | not reproducible in this environment | environment-dependent (cause undetermined) — feature confirmed working | RESEARCH.md's raw capture: `GCP Cloud KMS 403 (IAM permission denied) must produce a scan_error`. **Not reproducible here**: `pytest tests/test_posture_scorefix125.py -q -m ""` passes 3/3 (all tests in the file, including this one) in isolation, inside the combined 4-file Task-2 run, and inside a `tests/test_p*.py` (143-test) run. Read the test and `quirk/scanner/gcp_connector.py::_scan_kms`: the module docstring is labeled "RED scaffolds" from Phase 125's original scaffolding commit, but POSTURE-02's fix (emitting a `scan_error` CryptoEndpoint on a 403/`HttpError`) is **already implemented and working** in this sandbox's current `gcp_connector.py`. No silent-swallow regression exists currently — left unmarked per the not-reproducible precedent rather than quarantining a passing, correctly-behaving test | no — test passes, no quarantine needed |
| `test_posture_scorefix125.py::test_gcp_sql_403_emits_scan_error` | not reproducible in this environment | environment-dependent (cause undetermined) — feature confirmed working | Same investigation and determination as the row above (`_scan_cloud_sql`'s 403 handling) — passes cleanly in every reproduction attempt; POSTURE-02's fix is already in place and functioning correctly | no — test passes, no quarantine needed |
