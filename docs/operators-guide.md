...
| `quirk-scanner[identity]` | impacket, ldap3 | Kerberos / AD CS scanners (install separately — not in `[all]`). SAML / DNSSEC need no extra: `signxml`, `lxml` and `dnspython` are core dependencies. |
