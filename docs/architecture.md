...
- **Migrations:** `quirk/db.py` exposes idempotent additive `_ensure_*_columns(...)` migrations (`_ensure_identity_columns`, `_ensure_gcp_columns`, `_ensure_v43_columns`, `_ensure_email_columns`, `_ensure_broker_columns`, `_ensure_phase41_columns`, `_ensure_phase46_columns`). They are **column-add only** — never drop, never rename — which keeps an older database file readable by a newer QUIRK without an offline migration step.
