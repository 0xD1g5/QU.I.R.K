...
## Phase 95 — Code-Signing Certificate Inventory (CSIGN-01..03, SCORE-01, LAB-01)

**Last Updated:** 2026-05-23

Adds code-signing certificate discovery to QU.I.R.K.: the `--inventory-code-signing` CLI flag
enumerates AD `userCertificate` LDAP attributes (EKU filter: Code Signing OID `1.3.6.1.5.5.7.3.3`),
classifies RSA < 2048-bit / EC < 256-bit / SHA-1 certificates as HIGH-severity
`CODE-SIGN/weak-algorithm` findings, and registers them in the CBOM with SHA-256 fingerprint
dedup. An in-process TLS EKU check runs even without `codesign_targets` configured.
`SCORE_WEIGHTS` advances 293.0→299.0 (+6.0 `agility_codesign_weak_algo_ratio`). The `ldaps`
chaos lab profile gains a `ldaps-codesign-seed` sidecar seeding `uid=codesign-weak` with an
RSA-1024/SHA-1 CodeSigning EKU certificate for end-to-end lab validation.
