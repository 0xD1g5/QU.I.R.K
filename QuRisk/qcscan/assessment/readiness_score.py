...
def _context_risk_multiplier(ctx: Dict[str, Any]) -> Tuple[int, List[Tuple[str, int]]]:
    """
    Returns (penalty_points, driver_list) to apply to overall score.
    Penalties are modest but meaningful in v3.5.1.
    """
    drivers: List[Tuple[str, int]] = []
    penalty = 0
    ...
    # Longevity weighting (HNDL risk grows with longevity)
    if longevity >= 10:
        penalty += 5
...
def compute_readiness_score(cfg, endpoints, findings) -> ReadinessScore:
    """
    v3.5.1 = v3.5 score + operator context weighting.
    """
    ctx = get_context(cfg)
    ...
    for e in tls_ok:
        ...
        if ver in ("TLSv1", "TLSv1.1"):
            ...
            drivers.append((f"Deprecated TLS version in use ({ver})", 8))
        elif ver == "TLSv1.2":
    ...
    # --- v3.5.1 Context weighting (modest penalty on score) ---
    ctx_penalty, ctx_drivers = _context_risk_multiplier(ctx)
    if ctx_penalty:
        total = _clamp(total - ctx_penalty, 0, 100)
        drivers.extend(ctx_drivers)

    # Crown jewels bump: if specified, increase urgency if they appear in findings
