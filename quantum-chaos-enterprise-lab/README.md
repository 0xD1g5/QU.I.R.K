...
| segmented-network | segnet-gateway, segnet-live-tls, segnet-live-ssh, segnet-prober | none published (in-container access only) | [Expected Findings](expected_results_segmented_network.md) | v5.12 (Phase 152, DISC-09/DISC-10); realistic routed-segment unreachable-host lab for chunked discovery / partial-result tolerance and the DISC-10 nmap timing-engine empirical check. Two custom bridges — segnet-live (10.70.0.0/24, real reused TLS+SSH services) and segnet-dead (10.71.0.0/24, no containers) — joined by segnet-gateway (`alpine:3.20` + iptables), which REJECTs all FORWARD traffic to the dead subnet with `tcp-reset`/`icmp-host-unreachable` instead of silence. **Published Ports diverges from every other row (none)** — macOS Docker Desktop cannot route host traffic into custom bridges, so all scans run via `docker compose exec segnet-prober nmap ...` from segnet-prober (`sensor.Dockerfile`, joined to segnet-live only + explicit route to segnet-dead via the gateway — see chaos-lab.md §3.24 for why prober cannot join segnet-dead directly). Dead-range verification target: 10.71.0.0/26 (62 usable addrs); the gateway's own 10.71.0.2 is excluded from the automated REJECT-rule sweep (a probe to .2 hits the gateway's own INPUT chain, not the FORWARD-chain REJECT rule under test), leaving 61 addresses genuinely REJECT-verified via `compare_discovery.py` — a raw nmap sweep of the full /26 (64 addresses) independently confirmed 63/64 REJECT-rule hits with .2 correctly excluded as gateway-self. A scaled reproduction of the original ~1024-host batch, not a 1:1 replica; the REJECT rule covers the whole /24 so no per-dead-host container is needed. lab.sh `_derive_all_profiles()` auto-discovers — no ALL_PROFILES edit needed. |
| multihost | mh-edge-legacy, mh-edge-expired, mh-app-crownjewel, mh-db-finance, mh-cache-session, mh-identity-dc, mh-saml-idp, mh-storage-archive, mh-storage-seed, mh-pki-ca, mh-ssh-jump, mh-edge-rsa1024, mh-edge-sha1, mh-edge-chainbroken, mh-legacy-intranet, mh-intranet-wiki, mh-intranet-tickets, mh-intranet-jenkins, mh-intranet-fileshare, mh-intranet-printsrv, mh-intranet-monitoring, mh-vpn-gateway, mh-mail-relay, mh-vendor-portal, mh-backup-console, mh-devtest-api, mh-staging-web, mh-iot-controller, mh-erp-frontend, mh-hr-portal, mh-payroll, mh-db-hr, mh-prober (33 services: 31 target hosts + mh-storage-seed + mh-prober) | none published (in-container access only) | [Expected Findings](expected_results_v4.md#profile-multihost) | 999.110 (2026-09-13; estate grown to 31 targets by 2026-09-14); THIRTY-ONE INDEPENDENT HOSTS on a third custom bridge `labnet` (10.80.0.0/24), each carrying a different crypto posture chosen to span the scanner's finding domains: TLS-weak (.10), expired-cert (.11), modern-TLS **crown jewel** (.20), Postgres at-rest (.30), Redis broker (.31), LDAP identity (.40), SAML identity (.41), S3 at-rest with one UNENCRYPTED bucket (.50), step-ca PKI (.60), SSH (.70), plus mh-prober (.200). **Additive by construction** — re-homes nothing, and no other profile's ports, services or oracle expectations change. This is the demonstrable slice of 999.110, NOT the full item: the full shape re-homes all 90 services onto logical hosts and rewrites every `expected_results_*.md` per-host, which is effort M. **Published Ports is `none` on purpose** (same as segmented-network) — `expose:` only, so the 31 hosts cannot collapse back onto 127.0.0.1 and cannot collide with the ~60 host ports the other profiles claim; macOS Docker Desktop cannot route into custom bridges, so scans run from mh-prober. Generalises the segmented-network static-`ipv4_address` precedent. **Live run 2026-09-14 (scoring v3): 400 findings, 5 CRITICAL / 14 HIGH / 33 MEDIUM / 16 LOW / 332 INFO across 37 hosts (31 subnet targets + 6 connector pseudo-hosts), readiness 15/100 POOR** (computed 61, capped by 5 open CRITICALs); **28 of 31 hosts carry actionable findings** — Postgres/LDAP/SSH return INFO only, recorded as an honest limitation in the oracle rather than papered over. The estate has grown past the original thin slice to **31 targets**; the oracle's per-host table covers all of them from measured output as of 2026-09-14. NOTE: `lab.sh up` does not pass `--build`, so a stale `chaoslab-mh-prober` image silently reproduces the superseded v2 score of 91/100 — rebuild with `docker compose -p chaoslab --profile multihost build mh-prober` before quoting any score. Scan: `docker exec chaoslab-mh-prober-1 quirk --config /scan-config.yaml --allow-internal-targets --allow-cleartext-broker-probe`. lab.sh `_derive_all_profiles()` auto-discovers — no ALL_PROFILES edit needed. |
