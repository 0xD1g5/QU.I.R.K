...
| multihost | mh-edge-legacy, mh-edge-expired, mh-app-crownjewel, mh-db-finance, mh-cache-session, mh-identity-dc, mh-saml-idp, mh-storage-archive, mh-storage-seed, mh-pki-ca, mh-ssh-jump, mh-prober | none published (in-container access only) | [Expected Findings](expected_results_v4.md#profile-multihost) | 999.110 **thin slice** (2026-09-13); TEN INDEPENDENT HOSTS on a third custom bridge `labnet` (10.80.0.0/24), each carrying a different crypto posture chosen to span the scanner's finding domains: TLS-weak (.10), expired-cert (.11), modern-TLS **crown jewel** (.20), Postgres at-rest (.30), Redis broker (.31), LDAP identity (.40), SAML identity (.41), S3 at-rest with one UNENCRYPTED bucket (.50), step-ca PKI (.60), SSH (.70), plus mh-prober (.200). **Additive by construction** — re-homes nothing, and no other profile's ports, services or oracle expectations change. This is the demonstrable slice of 999.110, NOT the full item: the full shape re-homes all 90 services onto logical hosts and rewrites every `expected_results_*.md` per-host, which is effort M. **Published Ports is `none` on purpose** (same as segmented-network) — `expose:` only, so ten hosts cannot collapse back onto 127.0.0.1 and cannot collide with the ~60 host ports the other profiles claim; macOS Docker Desktop cannot route into custom bridges, so scans run from mh-prober. Generalises the segmented-network static-`ipv4_address` precedent. Live run 2026-09-13: 106 findings, 1 CRITICAL / 3 HIGH / 10 MEDIUM, readiness 89/100, **7 of 10 hosts with actionable findings** — Postgres/LDAP/SSH return INFO only, recorded as an honest limitation in the oracle rather than papered over. Scan: `docker exec chaoslab-mh-prober-1 quirk --config /scan-config.yaml --allow-internal-targets --allow-cleartext-broker-probe`. lab.sh `_derive_all_profiles()` auto-discovers — no ALL_PROFILES edit needed. |
